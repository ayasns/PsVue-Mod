...
(function () {
  ...
  // --- AES keys (user-provided) - in-memory only (if you used AES support earlier)
  var __aes_key_utf8 = "SENTV0ASDKFGJJLFJSJKLFJKOEKFSPKP";
  var __aes_iv_b64 = "gI1zB0GB+Z5AiNhwZXeKZw==";

  // helpers: base64 -> Uint8Array / utf8 -> Uint8Array / zeroing
    ...
    try {
      if (typeof atob === 'function') {
        ...
        for (var i = 0; i < b64.length; i += 4) {
          ...
          if (encoded3 !== 64 && !isNaN(encoded3)) {
            bytes[p++] = ((encoded2 & 15) << 4) | (encoded3 >> 2);
          }
          if (encoded4 !== 64 && !isNaN(encoded4)) {
  ...
  function utf8ToBytes(s) {
    try {
      ...
      for (var i = 0; i < s.length; i++) {
        ...
        else if (code < 0xd800 || code >= 0xe000) { arr.push(0xe0 | (code >> 12)); arr.push(0x80 | ((code >> 6) & 0x3f)); arr.push(0x80 | (code & 0x3f)); }
        else { i++; var code2 = s.charCodeAt(i); var full = 0x10000 + (((code & 0x3ff) << 10) | (code2 & 0x3ff)); arr.push(0xf0 | (full >> 18)); arr.push(0x80 | ((full >> 12) & 0x3f)); arr.push(0x80 | ((full >> 6) & 0x3f)); arr.push(0x80 | (full & 0x3f)); }
      }
      return new Uint8Array(arr);
    } catch (e) { return new Uint8Array(0); }
  }

  function zeroArray(u8) {
    try { if (!u8) return; for (var i = 0; i < u8.length; i++) u8[i] = 0; } catch (e) {}
  }
  ...
  function arrayBufferToString(buf) {
    ...
    try { return (typeof TextDecoder !== 'undefined') ? new TextDecoder('utf-8').decode(arr) : decodeURIComponent(escape(s)); } catch (e) { return s; }
  }

  // read raw bytes of a file into a Uint8Array (in-memory only)
  function readFileBytes(path, callback) {
    try {
      ...
      var statMax = 1024 * 1024 * 16; // 16MB cap for safety; adjust if needed
      var maxRead = statMax;
      var _buf = mem.malloc(maxRead);
      if (!_buf) { try { fn.close_sys(fd); } catch (e) {} return callback(new Error('Out of memory')); }
      var read_len = fn.read_sys(fd, _buf, new BigInt(0, maxRead));
      try { fn.close_sys(fd); } catch (e) {}
      var len = safeBigIntToNumber(read_len);
      if (len <= 0) { try { if (typeof mem.free === 'function') mem.free(_buf); } catch (e) {} return callback(null, new Uint8Array(0)); }
      var bytes = new Uint8Array(len);
      for (var i = 0; i < len; i++) {
        try { bytes[i] = mem.view(_buf).getUint8(i); } catch (e) { bytes[i] = 0; }
      }
      try { if (typeof mem.free === 'function') mem.free(_buf); } catch (e) {}
      _buf = null;
  ...
  var modalVideo = null;
  var modalAudio = null;
  var modalControls = null; // declared early so guarded writes won't crash
  var modalJsScrollOffset = 0; // for JS viewer scrolling

  // Zoom state (R2 analog)
  var currentZoom = 1.0;
  var maxZoom = 1.6; // max scale when R2 fully pressed
  var zoomLerpSpeed = 0.25; // smoothing factor
  ...
  function pushCreated(el) { try { if (!el) return; createdElements.push(el); if (createdElements.length > 2000) createdElements.splice(0, 200); } catch (e) {} }
  function hideCreated() { for (var i = 0; i < createdElements.length; i++) { try { if (createdElements[i]) createdElements[i].visible = false; } catch (e) {} } }
  function showCreated() { for (var i = 0; i < createdElements.length; i++) { try { if (createdElements[i]) createdElements[i].visible = true; } catch (e) {} } }
  function clearCreated() { try { createdElements = []; } catch (e) { createdElements = []; } }

  // apply zoom (best-effort)
  function applyZoom(z) {
    try {
      // prefer a root-level scale if supported
      if (typeof jsmaf !== 'undefined' && jsmaf.root && typeof jsmaf.root.scaleX !== 'undefined') {
        try { jsmaf.root.scaleX = z; jsmaf.root.scaleY = z; return; } catch (e) {}
      }
      // fallback: scale background/logo and all created elements
      try { if (background) { background.scaleX = z; background.scaleY = z; } } catch (e) {}
      try { if (logo) { logo.scaleX = z; logo.scaleY = z; } } catch (e) {}
      // scale created elements (guarded)
      for (var i = 0; i < createdElements.length; i++) {
        try {
          var el = createdElements[i];
          if (!el) continue;
          if (typeof el.scaleX !== 'undefined') {
            try { el.scaleX = z; el.scaleY = z; } catch (e) {}
          }
        } catch (e) {}
      }
    } catch (e) {}
  ...
  try { new Style({ name: 'title', color: 'white', size: 40 }); } catch (e) {}
  try { new Style({ name: 'small', color: 'white', size: 22 }); } catch (e) {}

  var background = null;
  try { background = new Image({ url: 'file:///../download0/img/multiview_bg_VAF.png', x: 0, y: 0, width: 1920, height: 1080 }); background.alpha = 0; background._baseX = background.x; if (jsmaf && jsmaf.root) jsmaf.root.children.push(background); } catch (e) { background = { alpha: 1, _baseX: 0 }; }

  var logo = null;
  try { logo = new Image({ url: 'file:///../download0/img/logo.png', x: 1620, y: 0, width: 300, height: 169 }); logo.alpha = 0; logo.scaleX = 0.98; logo.scaleY = 0.98; if (jsmaf && jsmaf.root) jsmaf.root.children.push(logo); } catch (e) { logo = null; }
  ...
  if (typeof useImageText !== 'undefined' && useImageText) {
    try {
      ...
      titleImg.alpha = 0; if (jsmaf && jsmaf.root) jsmaf.root.children.push(titleImg);
    } catch (e) {}
  } else {
    try {
      ...
      if (jsmaf && jsmaf.root) jsmaf.root.children.push(titleText);
    } catch (e) { titleText = null; }
  }

  // controls: search placeholder and sort/breadcrumb
  var searchBox = null;
  try { searchBox = new Image({ url: 'file:///assets/img/button_over_9.png', x: 130, y: 140, width: 420, height: 56 }); searchBox.alpha = 0.95; if (jsmaf && jsmaf.root) jsmaf.root.children.push(searchBox); } catch (e) { searchBox = null; }
  var searchText = null;
  try { searchText = new jsmaf.Text(); searchText.text = 'Search: (Triangle to clear)'; searchText.x = 140; searchText.y = 152; searchText.style = 'white'; if (jsmaf && jsmaf.root) jsmaf.root.children.push(searchText); } catch (e) { searchText = null; }
  var sortText = null;
  try { sortText = new jsmaf.Text(); sortText.text = 'Sort: ' + sortMode; sortText.x = 580; sortText.y = 152; sortText.style = 'white'; if (jsmaf && jsmaf.root) jsmaf.root.children.push(sortText); } catch (e) { sortText = null; }
  var breadcrumbText = null;
  try { breadcrumbText = new jsmaf.Text(); breadcrumbText.text = currentPath; breadcrumbText.x = 130; breadcrumbText.y = 190; breadcrumbText.style = 'small'; if (jsmaf && jsmaf.root) jsmaf.root.children.push(breadcrumbText); } catch (e) { breadcrumbText = null; }

  // modal overlay and container (bigger)
  try { modalOverlay = new Image({ url: 'file:///assets/img/modal_overlay.png', x: 0, y: 0, width: 1920, height: 1080 }); modalOverlay.alpha = 0.55; modalOverlay.visible = false; if (jsmaf && jsmaf.root) jsmaf.root.children.push(modalOverlay); } catch (e) { modalOverlay = null; }

  try { modal = new Image({ url: 'file:///assets/img/panel_bg.png', x: 120, y: 60, width: 1680, height: 960 }); modal.visible = false; if (jsmaf && jsmaf.root) jsmaf.root.children.push(modal); } catch (e) { modal = null; }

  try { modalHeader = new jsmaf.Text(); modalHeader.text = ''; modalHeader.x = 200; modalHeader.y = 90; modalHeader.style = 'title'; modalHeader.alpha = 0; if (jsmaf && jsmaf.root) jsmaf.root.children.push(modalHeader); } catch (e) { modalHeader = null; }

  try { modalCloseBtn = new Image({ url: 'file:///assets/img/button_over_9.png', x: 1600, y: 100, width: 120, height: 46 }); modalCloseBtn.visible = false; if (jsmaf && jsmaf.root) jsmaf.root.children.push(modalCloseBtn); } catch (e) { modalCloseBtn = null; }

  try { modalCloseText = new jsmaf.Text(); modalCloseText.text = 'Close'; modalCloseText.x = 1630; modalCloseText.y = 110; modalCloseText.style = 'white'; modalCloseText.visible = false; if (jsmaf && jsmaf.root) jsmaf.root.children.push(modalCloseText); } catch (e) { modalCloseText = null; }

  try { modalContentText = new jsmaf.Text(); modalContentText.text = ''; modalContentText.x = 200; modalContentText.y = 160; modalContentText.style = 'small'; modalContentText.alpha = 0; modalContentText.wrap = true; if (jsmaf && jsmaf.root) jsmaf.root.children.push(modalContentText); } catch (e) { modalContentText = null; }

  try { modalImage = new Image({ url: '', x: 240, y: 200, width: 1440, height: 720 }); modalImage.visible = false; if (jsmaf && jsmaf.root) jsmaf.root.children.push(modalImage); } catch (e) { modalImage = null; }

  try { modalControls = new jsmaf.Text(); modalControls.text = ''; modalControls.x = 240; modalControls.y = 940; modalControls.style = 'white'; modalControls.alpha = 0; if (jsmaf && jsmaf.root) jsmaf.root.children.push(modalControls); } catch (e) { modalControls = null; }

  // cursor
  try { cursor = new Image({ url: 'file:///assets/img/cursor.png', x: virtualMouse.x - cursorSize.w/2, y: virtualMouse.y - cursorSize.h/2, width: cursorSize.w, height: cursorSize.h }); cursor.visible = false; if (jsmaf && jsmaf.root) jsmaf.root.children.push(cursor); } catch (e) { cursor = { visible: false, x: virtualMouse.x, y: virtualMouse.y }; }
  ...
  function animate(obj, from, to, duration, onStep, done) {
    ...
    var id = _setInterval(function () {
      ...
      if (onStep) onStep(e);
      if (t >= 1) {
        try { if (jsmaf && typeof jsmaf.clearInterval === 'function') jsmaf.clearInterval(id); else clearInterval(id); } catch (ex) {}
        if (done) done();
      }
    }, step);
    return id;
  ...
  function startOrangeDotLoop() {
    if (_markerPulseInterval) try { if (jsmaf && typeof jsmaf.clearInterval === 'function') jsmaf.clearInterval(_markerPulseInterval); else clearInterval(_markerPulseInterval); } catch (e) {}
    var phase = 0;
    try {
      _markerPulseInterval = (jsmaf && typeof jsmaf.setInterval === 'function') ? jsmaf.setInterval(function () {
        phase += 0.06;
  ...
  function startLogoLoop() {
    var phase = 0;
    if (_logoAnimInterval) try { if (jsmaf && typeof jsmaf.clearInterval === 'function') jsmaf.clearInterval(_logoAnimInterval); else clearInterval(_logoAnimInterval); } catch (e) {}
    try {
      _logoAnimInterval = (jsmaf && typeof jsmaf.setInterval === 'function') ? jsmaf.setInterval(function () {
        phase += 0.02;
        try { if (logo) { logo.y = Math.sin(phase) * 6; logo.scaleX = 0.99 + Math.sin(phase * 0.9) * 0.01; logo.scaleY = logo.scaleX; } } catch (e) {}
  ...
  function fadeOutBgm(duration, done) {
    ...
    if (_bgmFadeInterval) try { if (jsmaf && typeof jsmaf.clearInterval === 'function') jsmaf.clearInterval(_bgmFadeInterval); else clearInterval(_bgmFadeInterval); } catch (e) {}
    var elapsed = 0; var step = 50; var dur = duration || 600;
    _bgmFadeInterval = (jsmaf && typeof jsmaf.setInterval === 'function') ? jsmaf.setInterval(function () {
      elapsed += step; var t = Math.min(elapsed / dur, 1);
      try { bgm.volume = Math.max(0, 0.45 * (1 - t)); } catch (e) {}
      if (t >= 1) { try { if (jsmaf && typeof jsmaf.clearInterval === 'function') jsmaf.clearInterval(_bgmFadeInterval); else clearInterval(_bgmFadeInterval); } catch (e) {} _bgmFadeInterval = null; try { if (bgm && typeof bgm.pause === 'function') bgm.pause(); } catch (e) {} if (done) done(); }
  ...
  function fadeInBgm(duration, done) {
    ...
    _bgmFadeInterval = (jsmaf && typeof jsmaf.setInterval === 'function') ? jsmaf.setInterval(function () {
      ...
      if (t >= 1) { try { if (jsmaf && typeof jsmaf.clearInterval === 'function') jsmaf.clearInterval(_bgmFadeInterval); else clearInterval(_bgmFadeInterval); } catch (e) {} _bgmFadeInterval = null; if (done) done(); }
    }, step) : setInterval(function () {
      elapsed += step; var t = Math.min(elapsed / dur, 1);
      try { bgm.volume = Math.min(0.45, 0.45 * t); } catch (e) {}
      if (t >= 1) { try { if (jsmaf && typeof jsmaf.clearInterval === 'function') jsmaf.clearInterval(_bgmFadeInterval); else clearInterval(_bgmFadeInterval); } catch (e) {} _bgmFadeInterval = null; if (done) done(); }
  ...
  function startBgm() { try { if (bgm && typeof bgm.play === 'function') bgm.play(); } catch (e) {} fadeInBgm(900); }
      ...
      if (rlen > 0) {
        ...
        while (offset < rlen) {
          try {
            ...
            offset += reclen || 1;
            if (offset > rlen + 4096) break; // safety stop
          } catch (e) { break; }
        }
      }
      try { fn.close_sys(fd); } catch (e) {}
  ...
  function buildGrid() {
    ...
    try { if (modalOverlay) modalOverlay.visible = false; } catch (e) {}
    try { if (modal) modal.visible = false; if (modal) modal._open = false; } catch (e) {}
    try { if (modalHeader) modalHeader.alpha = 0; } catch (e) {}
    try { if (modalCloseBtn) modalCloseBtn.visible = false; } catch (e) {}
    try { if (modalCloseText) modalCloseText.visible = false; } catch (e) {}
    try { if (modalContentText) modalContentText.alpha = 0; } catch (e) {}
    try { if (modalControls) modalControls.alpha = 0; } catch (e) {}
    ...
    for (var i = 0; i < filteredEntries.length; i++) {
      ...
      try {
        ...
        icon.alpha = 0; if (jsmaf && jsmaf.root) jsmaf.root.children.push(icon); pushCreated(icon);
      } catch (e) {
        try {
      ...
      try {
        ...
        if (jsmaf && jsmaf.root) jsmaf.root.children.push(btn); pushCreated(btn); buttons.push(btn);
      ...
      try {
        ...
        if (jsmaf && jsmaf.root) jsmaf.root.children.push(marker); pushCreated(marker); buttonMarkers.push(marker);
      ...
      try {
        ...
        if (jsmaf && jsmaf.root) jsmaf.root.children.push(txt); pushCreated(txt); buttonTexts.push(txt);
      ...
      (function (b, t, ix, byy) {
        try {
          if (jsmaf && typeof jsmaf.setTimeout === 'function') {
            ...
            setTimeout(function () { try { if (b) animate(b, { alpha: 0, y: byy + 20 }, { alpha: 1, y: byy }, 360); } catch (e) {} try { if (t) animate(t, { alpha: 0, y: t.y + 20 }, { alpha: 1, y: t.y }, 360); } catch (e) {} }, 100 + ix * 30);
          }
        } catch (e) {}
      })(btn, txt, i, by);
    }
    ...
    if (currentPath !== startPath) {
      ...
      try {
        ...
        upBtn.alpha = 0; if (jsmaf && jsmaf.root) jsmaf.root.children.push(upBtn); pushCreated(upBtn);
        var upTxt = new jsmaf.Text(); upTxt.text = '.. (Up)'; upTxt.x = upBtnX + 10; upTxt.y = upBtnY + 6; upTxt.style = 'white'; upTxt.alpha = 0; if (jsmaf && jsmaf.root) jsmaf.root.children.push(upTxt); pushCreated(upTxt);
        upBtn._isUp = true;
    ...
    startOrangeDotLoop();
    startLogoLoop();
    try { if (breadcrumbText) breadcrumbText.text = currentPath; } catch (e) {}
    // ensure BGM running
    startBgm();

    // ensure applied zoom after grid built
    try { applyZoom(currentZoom); } catch (e) {}
  }
  ...
  function cleanupModalMedia() {
    ...
    try { if (modalImage) modalImage.visible = false; } catch (e) {}
    try { if (modalControls) modalControls.text = ''; } catch (e) {}
    try { if (modalContentText) modalContentText.text = ''; } catch (e) {}
  ...
  function showModalText(title, content) {
    ...
    hideCreated();
    try { if (modalOverlay) modalOverlay.visible = true; } catch (e) {}
    try { if (modal) { modal.visible = true; modal._open = true; } } catch (e) {}
    try { if (modalHeader) { modalHeader.text = title || ''; modalHeader.alpha = 1; } } catch (e) {}
    try { if (modalCloseBtn) modalCloseBtn.visible = true; } catch (e) {}
    try { if (modalCloseText) modalCloseText.visible = true; } catch (e) {}
    try { if (modalContentText) { modalContentText.text = content || ''; modalContentText.alpha = 1; } } catch (e) {}
    try { if (modalImage) modalImage.visible = false; } catch (e) {}
    try { if (modalControls) modalControls.alpha = 0; } catch (e) {}
  }
  ...
  function openImageViewer(item) {
    ...
    try {
      ...
      try { if (modalImage) { modalImage.url = url; modalImage.visible = true; } } catch (e) {}
      try { if (modalContentText) modalContentText.alpha = 0; } catch (e) {}
      try { if (modalHeader) modalHeader.text = item.name || 'Image'; } catch (e) {}
      try { if (modalOverlay) modalOverlay.visible = true; } catch (e) {}
      try { if (modal) { modal.visible = true; modal._open = true; } } catch (e) {}
      try { if (modalCloseBtn) modalCloseBtn.visible = true; } catch (e) {}
      try { if (modalCloseText) modalCloseText.visible = true; } catch (e) {}
      ...
      try {
        if (jsmaf && typeof jsmaf.setTimeout === 'function') {
          jsmaf.setTimeout(function () {
            try {
              if (!modalImage || !modalImage.visible) return;
  ...
  function openVideoViewer(item) {
    ...
    hideCreated();
    try { if (modalContentText) modalContentText.alpha = 0; } catch (e) {}
    try { if (modalHeader) modalHeader.text = item.name || 'Video'; } catch (e) {}
    try { if (modalOverlay) modalOverlay.visible = true; } catch (e) {}
    try { if (modal) { modal.visible = true; modal._open = true; } } catch (e) {}
    try { if (modalCloseBtn) modalCloseBtn.visible = true; } catch (e) {}
    try { if (modalCloseText) modalCloseText.visible = true; } catch (e) {}
    try { if (modalControls) { modalControls.alpha = 1; modalControls.text = 'Square: Play/Pause  Circle: Close'; } } catch (e) {}
    fadeOutBgm(500);
    try {
      if (typeof jsmaf !== 'undefined' && typeof jsmaf.VideoClip === 'function') {
        var url = item.path.indexOf('file://') === 0 ? item.path : ('file://' + item.path);
        modalVideo = new jsmaf.VideoClip({ url: url, x: 240, y: 200, width: 1440, height: 720 });
        modalVideo.visible = true;
        try { modalVideo.play(); try { if (modalControls) modalControls.text = 'Square: Play/Pause  Circle: Close'; } catch (e) {} } catch (e) { try { if (modalControls) modalControls.text = 'Video loaded'; } catch(e) {} }
      } else {
        try { if (modalControls) modalControls.text = 'Video playback not supported'; } catch(e) {}
        showModalText(item.name || 'Video', 'Playback not supported in this environment.');
        fadeInBgm(400);
      }
    } catch (e) {
      try { if (modalControls) modalControls.text = 'Video error'; } catch (er) {}
  ...
  function openAudioViewer(item) {
    ...
    hideCreated();
    try { if (modalContentText) modalContentText.alpha = 0; } catch (e) {}
    try { if (modalHeader) modalHeader.text = item.name || 'Audio'; } catch (e) {}
    try { if (modalOverlay) modalOverlay.visible = true; } catch (e) {}
    try { if (modal) { modal.visible = true; modal._open = true; } } catch (e) {}
    try { if (modalCloseBtn) modalCloseBtn.visible = true; } catch (e) {}
    try { if (modalCloseText) modalCloseText.visible = true; } catch (e) {}
    ...
    try {
      ...
      var url = item.path.indexOf('file://') === 0 ? item.path : ('file://' + item.path);
      modalAudio.open(url);
      try { modalAudio.play(); modalAudio._isPlaying = true; try { if (modalControls) modalControls.text = 'Square: Play/Pause  Circle: Close'; } catch(e) {} } catch (e) { try { if (modalControls) modalControls.text = 'Audio loaded'; } catch(e) {} }
    } catch (e) {
      try { if (modalControls) modalControls.text = 'Audio playback not supported'; } catch(e) {}
  ...
  function openJsViewer(item) {
    ...
    hideCreated();
    try {
      if (!item || !item.path) return showModalText('Preview error', 'Invalid file path');
      try { if (modalContentText) modalContentText.alpha = 1; } catch (e) {}
      try { if (modalHeader) modalHeader.text = item.name || 'JS File'; } catch (e) {}
      try { if (modalOverlay) modalOverlay.visible = true; } catch (e) {}
      try { if (modal) { modal.visible = true; modal._open = true; } } catch (e) {}
      try { if (modalCloseBtn) modalCloseBtn.visible = true; } catch (e) {}
      try { if (modalCloseText) modalCloseText.visible = true; } catch (e) {}
      try { if (modalControls) { modalControls.alpha = 1; modalControls.text = 'Up/Down: Scroll  Circle: Close'; } } catch (e) {}
      fadeOutBgm(400);
      ...
      previewFile(item.path, function (err, txt) {
        ...
        try {
          if (modalContentText) {
            // disable wrapping for code to preserve lines
            try { modalContentText.wrap = false; } catch (e) {}
            ...
            showModalText(item.name || 'File', txt || '(empty)');
          }
        } catch (e) {
          showModalText('JS error', 'Could not display file: ' + (e.message || e));
          fadeInBgm(400);
        }
      });
    } catch (e) { showModalText('JS view error', 'Could not display file: ' + (e.message || e)); fadeInBgm(400); }
  }

  // --- AES (.aes) viewer (if you used AES earlier)
  function openAesViewer(item) {
    try {
      cleanupModalMedia();
      hideCreated();
      if (!item || !item.path) return showModalText('AES error', 'Invalid file path');

      readFileBytes(item.path, function (err, encBytes) {
        if (err) { showModalText('AES read error', 'Could not read file: ' + (err.message || err)); return; }
        var keyBytes = utf8ToBytes(__aes_key_utf8);
        var ivBytes = base64ToBytes(__aes_iv_b64);

        if (typeof crypto !== 'undefined' && crypto.subtle && typeof crypto.subtle.importKey === 'function') {
          try {
            crypto.subtle.importKey('raw', keyBytes.buffer || keyBytes, { name: 'AES-CBC' }, false, ['decrypt']).then(function (keyObj) {
              return crypto.subtle.decrypt({ name: 'AES-CBC', iv: ivBytes.buffer || ivBytes }, keyObj, encBytes.buffer || encBytes);
            }).then(function (plainBuf) {
              try {
                var plainText = arrayBufferToString(plainBuf);
                try { if (modalOverlay) modalOverlay.visible = true; } catch (e) {}
                try { if (modal) { modal.visible = true; modal._open = true; } } catch (e) {}
                try { if (modalHeader) { modalHeader.text = item.name || 'Decrypted'; modalHeader.alpha = 1; } } catch (e) {}
                try { if (modalCloseBtn) modalCloseBtn.visible = true; } catch (e) {}
                try { if (modalCloseText) modalCloseText.visible = true; } catch (e) {}
                try {
                  if (modalContentText) {
                    try { modalContentText.wrap = true; } catch (e) {}
                    modalContentText.text = plainText || '(empty)';
                    modalContentText.alpha = 1;
                    modalContentText.y = 160;
                  } else {
                    showModalText(item.name || 'Decrypted', plainText || '(empty)');
                  }
                } catch (e) {}
                try { if (modalControls) { modalControls.alpha = 1; modalControls.text = 'Decrypted in-memory only. Up/Down to scroll.'; } } catch (e) {}
                fadeOutBgm(400);
                try { zeroArray(keyBytes); } catch (e) {}
                try { zeroArray(ivBytes); } catch (e) {}
                try { zeroArray(new Uint8Array(plainBuf)); } catch (e) {}
                try { zeroArray(encBytes); } catch (e) {}
              } catch (e) { showModalText('AES decode error', 'Could not decode plaintext: ' + (e.message || e)); }
            }).catch(function (err2) {
              showModalText('AES decrypt error', 'Decryption failed: ' + (err2 && (err2.message || err2)));
              try { zeroArray(keyBytes); } catch (e) {}
              try { zeroArray(ivBytes); } catch (e) {}
              try { zeroArray(encBytes); } catch (e) {}
            });
          } catch (e) {
            showModalText('AES error', 'Decryption setup failed: ' + (e && e.message));
            try { zeroArray(keyBytes); } catch (er) {}
            try { zeroArray(ivBytes); } catch (er) {}
            try { zeroArray(encBytes); } catch (er) {}
          }
        } else {
          showModalText('AES unsupported', 'Crypto API not available in this environment. Decryption requires WebCrypto (crypto.subtle).');
          try { zeroArray(keyBytes); } catch (e) {}
          try { zeroArray(ivBytes); } catch (e) {}
          try { zeroArray(encBytes); } catch (e) {}
        }
      });
    } catch (e) { showModalText('AES view error', 'Could not open AES file: ' + (e && e.message)); }
  }
    ...
    try {
      if (modalVideo) {
        try {
          ...
          } else { try { modalVideo.play(); } catch (e) {} }
        } catch (e) {}
        return;
      }
    } catch (e) {}
  ...
  function closeModalAndResumeBgm() {
    cleanupModalMedia();
    try { if (modalOverlay) modalOverlay.visible = false; } catch (e) {}
    try { if (modal) { modal.visible = false; modal._open = false; } } catch (e) {}
    try { if (modalHeader) modalHeader.alpha = 0; } catch (e) {}
    try { if (modalCloseBtn) modalCloseBtn.visible = false; } catch (e) {}
    try { if (modalCloseText) modalCloseText.visible = false; } catch (e) {}
    try { if (modalContentText) modalContentText.alpha = 0; } catch (e) {}
  ...
  function showCursor() {
    try {
      ...
      if (mouseHideTimeout) try { if (jsmaf && typeof jsmaf.clearTimeout === 'function') jsmaf.clearTimeout(mouseHideTimeout); else clearTimeout(mouseHideTimeout); } catch (e) {}
      mouseHideTimeout = (jsmaf && typeof jsmaf.setTimeout === 'function') ? jsmaf.setTimeout(function () {
        if (Date.now() - lastRealMouseTime >= mouseInactivityMs) try { cursor.visible = false; } catch(e) {}
        mouseHideTimeout = null;
      }, mouseInactivityMs) : setTimeout(function () { if (Date.now() - lastRealMouseTime >= mouseInactivityMs) try { cursor.visible = false; } catch(e) {} mouseHideTimeout = null; }, mouseInactivityMs);
    } catch (e) {}
  }
  ...
  function updateCursorPosition(x, y) {
    ...
    lastRealMouseTime = Date.now();
    try { if (!cursor.visible) cursor.visible = true; } catch (e) {}
  }
  ...
  jsmaf.onMouseDown = function (mx, my, btn) {
    ...
    if (modal && modal._open) {
      try {
        if (modalCloseBtn && modalCloseBtn.visible && mx >= modalCloseBtn.x && my >= modalCloseBtn.y && mx <= modalCloseBtn.x + modalCloseBtn.width && my <= modalCloseBtn.y + modalCloseBtn.height) {
    ...
    // search box click: show simple hint
    try {
      if (searchBox && mx >= searchBox.x && my >= searchBox.y && mx <= searchBox.x + searchBox.width && my <= searchBox.y + searchBox.height) {
    ...
    // IMPORTANT: if a modal/viewer is open, we do NOT process grid clicks.
    // This prevents accidental activations while viewing content (X/Cross should do nothing in-view).
    try {
      if (modal && modal._open) {
        return;
      }
  ...
  function navigateUp() {
    try {
      ...
      if (modal && modal._open) { closeModalAndResumeBgm(); return; }
  ...
  jsmaf.onKeyDown = function (keyCode) {
    try {
      ...
      if (keyCode === 19 || keyCode === 20 || keyCode === 2) {
        ...
        if (modal && modal._open && modalContentText && modalContentText.alpha === 1) {
          modalJsScrollOffset -= 28;
          try { modalContentText.y = 160 + modalJsScrollOffset; } catch (e) {}
          return;
        }
      ...
      // Confirm / Cross (X) — DISABLED while modal/viewer open (so X does nothing in-view).
      if (keyCode === 14) {
        // If a modal/viewer is open, ignore Cross to prevent accidental actions while viewing content.
        try {
          if (modal && modal._open) {
            return;
          }
        } catch (e) {}
        // When no modal is open, normal behavior:
        // If audio/video (modalAudio/modalVideo) are present and no modal open this is unlikely,
        // but keep existing behavior for general case.
        if (modal && modal._open && (modalAudio || modalVideo)) { toggleModalPlayPause(); return; }
      ...
      if (keyCode === 3 || keyCode === 12) {
        searchQuery = '';
        try { if (searchText) searchText.text = 'Search: (Triangle to clear)'; } catch (e) {}
        buildGrid();
        return;
      }
      ...
      if (keyCode === 27) {
        if (modal && modal._open) closeModalAndResumeBgm();
        return;
      }
    } catch (e) {
      // tolerate errors
  ...
  (function startGamepadMouse() {
    ...
    try {
      gpPollInterval = _setInterval(function () {
        try {
          ...
          // R2 analog value (commonly button index 7) - pressure sensitive zoom
          var r2Value = 0;
          try {
            if (gp.buttons && gp.buttons.length > 7 && gp.buttons[7]) {
              var b7 = gp.buttons[7];
              r2Value = (typeof b7.value !== 'undefined') ? b7.value : (b7.pressed ? 1.0 : 0.0);
            } else if (gp.buttons && gp.buttons.length > 6 && gp.buttons[6]) {
              // some mappings swap triggers; fall back to button 6
              var b6 = gp.buttons[6];
              r2Value = (typeof b6.value !== 'undefined') ? b6.value : (b6.pressed ? 1.0 : 0.0);
            }
            if (typeof r2Value !== 'number' || isNaN(r2Value)) r2Value = 0;
          } catch (e) { r2Value = 0; }

          // Compute target zoom and lerp
          try {
            var targetZoom = 1.0 + r2Value * (maxZoom - 1.0); // 0->1.0, 0.5->mid, 1->max
            // smoothing
            ...
            try { applyZoom(currentZoom); } catch (e) {}
            // optionally show a subtle indicator in modalControls (guarded)
            try {
              if (modalControls && modalControls.alpha === 1) {
                modalControls.text = 'Zoom: ' + currentZoom.toFixed(2) + ' (R2 ' + Math.round(r2Value * 100) + '%)';
              }
            } catch (e) {}
          } catch (e) {}

          // emulate click on Cross (0) -- routed through onMouseDown to centralize handling
          if (pressed0 && !lastGpButtons[0]) {
            try {
              // If modal open, prevent click activation (onMouseDown will early-return)
              jsmaf.onMouseDown(Math.round(virtualMouse.x), Math.round(virtualMouse.y), 0);
            } catch (e) {}
          }
          ...
          if (pressed2 && !lastGpButtons[2]) {
            try { if (modal && modal._open && (modalAudio || modalVideo)) toggleModalPlayPause(); } catch (e) {}
          }
  ...
  function entrance() {
    try { if (background) animate(background, { alpha: 0 }, { alpha: 1 }, 800); } catch (e) {}
    ...
    try {
      if (jsmaf && typeof jsmaf.setTimeout === 'function') {
        ...
        setTimeout(function () { buildGrid(); enforceTextWhite(); }, 300);
      }
    } catch (e) { buildGrid(); enforceTextWhite(); }
  }
